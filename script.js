...
const calcAverageHumanAge = function (ages) {
    const hAge = ages.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
    const adult = hAge.filter(age => age >= 18);
    console.log(hAge);
    console.log(adult);
    const avghAge = adult.reduce((acc, age) => acc + age, 0) / adult.length;
    return avghAge;
}
...
//movements.filter(mov => mov > 0).map
...
console.log(converTitle('and here is a another title with an EXamplE'));
...
console.log(`Sarah's dog is eating ${dogSarah.curFood > dogSarah.recFood ? 'much' : 'normal'}`);
